                        ...
                        const int A_height, const int A_width, const int bitwidth)
{
    ...
    // printf("Decode_col--offset_input: %d\n", offset_input);
    ...
    for (int bid=blockIdx.x; bid<gdx*gdy; bid+=gridDim.x)
    {
        ...
        for (int bIdx = 0; bIdx < bitwidth; bIdx++){
            int r0 = A[bIdx*offset_input + (by*8+ly)*gdx*4 + bx*4 + lx];
                        ...
                        const int A_height, const int A_width, const int bitwidth)
{
    ...
    for (int bid=blockIdx.x; bid<gdx*gdy; bid+=gridDim.x)
    {
        ...
        for (int bIdx = 0; bIdx < bitwidth; bIdx++){
            int r0 = A[bIdx*offset_input + (bx*8+lx)*gdy*4 + by*4 + ly];
...
)
{
    ...
    // refer this nbits*STEP128(height)*4, PAD128(width) for Weights.
    const int opt_offset = STEP128(X_height)*PAD128(W_width)*128/32;
    // printf("MM_col--opt_offset: %d\n", opt_offset);
    ...
    const int gdh = STEP128(X_height);   // output height --> M
    const int gdm = STEP128(W_width);    // output width --> N
    ...
    for (int bid=blockIdx.x*warpPerBlock+warpid; bid<gdx*gdy; bid+=gridDim.x*warpPerBlock)
    {
        ...
        // rwo major output.
        const int bx = bid / gdy;       // vertical index.
        const int by = bid % gdy;       // horizeonal index.
        ...
        // finished one output tile and store to shared memory
        // * using column major here for shared output.
        store_matrix_sync(&Cs[warpid*64], c_frag, 8, wmma::mem_col_major);
        ...
        for (int bIdx = 0; bIdx < out_bit; bIdx++){
            ...
            // get the corresponding decomposed bit value.
            bool v0 = v0_in && (((Cs[warpid*64+laneid]>>bIdx) & 0x1) > 0);
            bool v1 = v1_in && (((Cs[warpid*64+4+laneid]>>bIdx) & 0x1) > 0);
            ...
            if (laneid < 4)
            {
                // * row major output. every 4 x (8x8) combined as int 32 along the y-axis. fixed y-axis.
                // Cb[(bx*8+laneid)*gdm*16+FLIPBITS(by,2)] = p0.elements[3-laneid]; 
                // Cb[(bx*8+4+laneid)*gdm*16+FLIPBITS(by,2)] = p1.elements[3-laneid]; 

                // printf("bx: %d, by: %d\n", bx, by);
                // * col major output. every 4 x (8x8) combined as int 32 along the y-axis. fixed y-axis.
                // printf("laneid: %d,\t FLIPBITS(by,2)*8+laneid)*gdx + bx: %d,\tFLIPBITS(by,2)*8+4+laneid)*gdx + bx: %d\n", laneid, (by*8+laneid)*gdx + bx, (by*8+4+laneid)*gdx + bx);

                Cb[(by*8+laneid)*gdh*16 + FLIPBITS(bx,2)] = p0.elements[3-laneid]; 
                Cb[(by*8+4+laneid)*gdh*16 + FLIPBITS(bx,2)] = p1.elements[3-laneid]; 
